...
class Calculator
{
    ...
    static public function calculateValancePrice(LineItem $orderLine)
    {
        ...
        $orderFabrics = $orderLine->order->fabrics;

        foreach($orderFabrics as $orderFabric) {
            if($orderFabric->type->isForValance()) {
                $fabricPrice = self::calculateFabricPricePerLineItem($orderLine, $orderFabric);
                $valancePrice += $fabricPrice;
            }
        }
    ...
    static public function calculateFabricPrice(LineItem $lineItem){
        ...
        foreach($orderFabrics as $orderFabric) {
            ...
            if(!$fabricTypeIsForValance) {
                $fabricPrice = self::calculateFabricPricePerLineItem($lineItem,$orderFabric);
    ...
    static private function calculateFabricPricePerLineItem(LineItem $lineItem, OrderFabric $orderFabric) {
        //alter height to account for cuts
        $cuts = static::fabricCuts($lineItem, $orderFabric);
        $cutLength = static::fabricCutLength($lineItem, $orderFabric);
