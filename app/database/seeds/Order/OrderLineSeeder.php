...
class OrderLineSeeder extends DatabaseSeeder
{
    public function run()
    {
        ...
        foreach($orderLines as $index => $orderLine){
            $previousOrderlineIsNotPartOfCurrentOrder = (isset($orderLines[$index-1]))?$orderLines[$index-1]['order_id'] != $orderLines[$index]['order_id']:null;

            if($index === 0 || $previousOrderlineIsNotPartOfCurrentOrder) {
                $lineNumber = 1;
            }
            else {
                $lineNumber++;
            }
            $orderLine['line_number'] = $lineNumber;
