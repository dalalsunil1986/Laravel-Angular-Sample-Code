...
class FabricController extends BaseController
{
    ...
    public function getIndex($id = null)
    {
        ...
        if(!$fabric instanceof \Fabric) {
            if($id === 'null'){
                $fabric = new Fabric;
                $fabric->owner_company_id = Company::where('name', '=', 'Unknown')->first()->id;
            } else {
    ...
    public function fabricSearch($string)
    {
        $fabrics = Fabric::where('color', 'like', '%'.$string.'%')
                         ->orWhere('pattern', 'like', '%'.$string.'%')
                         ->orWhere('sidemark', 'like', '%'.$string.'%')
                         ->orWhere('ticket_number', 'like', '%'.$string.'%')
    ...
    public function saveFabric($fabricId){
        ...
        $inputs = Input::all();
        $inputs['date_received'] = (isset($inputs['date_received']) && $inputs['date_received'] !== "")? date('Y-m-d', strtotime($inputs['date_received'])) : NULL;
