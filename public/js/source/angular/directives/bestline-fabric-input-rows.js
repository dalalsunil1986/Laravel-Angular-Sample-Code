...
.directive('bestlineFabricInputRows', function() {
    ...
    bestlineFabricInputRows.controller = function($scope, $q, $timeout, filterFilter, bestlineApi){
            ...
            function setFabricSelectOptions(fabricOptions){
               ...
               if(row.fabric_id){
                    ...
                    if(initialFabric){
                        ...
                        bestlineApi.fabric(row.fabric_id).get().then(getFabricSuccess);

                        function getFabricSuccess(response)
                        {
                            row.fabricSelect.selected = response.data;
                            row.fabricSelect.options.push(response.data);
                        }
                    }
               }
            }
        };
        bestlineFabricInputRowsController.checkForBestlineFabrics = function(row){

            return $q(checkForBestlineFabricsPromise);

            function checkForBestlineFabricsPromise(resolveCFBOP, rejectCFBOP){
                if(row.fabric_type_id){
                    if(!$scope.fabrics[row.fabric_type_id]){

                        bestlineFabricInputRowsController.getBestlineFabrics(row).then(resolveCFBOP);
                    } else {
                        resolveCFBOP($scope.fabrics[row.fabric_type_id]);
                    }
                } else {
                    resolveCFBOP([]);
                }
        ...
        bestlineFabricInputRowsController.getBestlineFabrics = function(orderFabric){

            return $q(getBestlineFabricsPromise);

            function getBestlineFabricsPromise(resolveGetBestlineFabrics, rejectGetBestlineFabrics){
                ...
                function getBestlineFabricsSuccess(response){
                    ...
                    resolveGetBestlineFabrics(response.data);
                };
                function getBestlineFabricsError(response){
                    orderFabric.fabricSelect.loading = false;
                    rejectGetBestlineFabrics(response);
                };
            };
        };
        ...
        bestlineFabricInputRowsController.viewApi = function(){
            ...
            $scope.fabricSearchChange = function(row, fabricSelect){
                if(fabricSelect.search != "") {
                    if (searchTextTimeout) $timeout.cancel(searchTextTimeout);
                    fabricSelect.fabricSearch = true;
                    searchTextTimeout = $timeout(function() {
                        bestlineApi.fabric().search(fabricSelect.search).then(getSearchFabricsSuccess);

                        function getSearchFabricsSuccess(response){

                            fabricSelect.options = response.data;
                            fabricSelect.fabricSearch = false;
                        }
                    }, 300);
                } else {
                    if (searchTextTimeout) $timeout.cancel(searchTextTimeout);
                    ...
                    function setFabricSelectOptions(fabricOptions){
                       ...
                       fabricSelect.options = fabricOptions;
                       fabricSelect.fabricSearch = false;
                    }
                }
            }
        ...
        bestlineFabricInputRowsController.getFabricSelectOptions = function(row){
            ...
            function getFabricSelectOptionsPromise(getFabricSelectOptionsResolve, getFabricSelectOptionsReject){
                ...
                bestlineFabricInputRowsController.checkForBestlineFabrics(row).then(checkForComFabrics);
                function checkForComFabrics(bestlineFabrics){
                    ...
                    if($scope.orderCompanyId){
                        ...
                        getFabricSelectOptionsResolve(bestlineFabrics);
                    }
                    function mergeComAndTypeFabrics(comFabrics){

                        var mergedFabrics = comFabrics.concat(bestlineFabrics);
